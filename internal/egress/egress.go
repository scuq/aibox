...
// ProjectFragments returns every project-*.acl fragment path, sorted. There is
// one shared squid on the internal network serving every project, so its
// allowlist is the union of ALL project fragments — not just the one for the
// current workspace. Composing only one project's fragment (or none, as
// `aibox net up` used to) is what made a domain allowed for one project vanish
// when another context regenerated the allowlist.
func ProjectFragments() []string {
	matches, _ := filepath.Glob(filepath.Join(ConfigDir(), "project-*.acl"))
...
//
//	base + enabled assistants + all project fragments + extra entries
//	→ normalise → validate → deduplicate
//
// Returned as file content; the caller writes it and gates the reload on
// `squid -k parse`.
func Compose(enabledAssistants []string, extra []string) (string, error) {
	var b strings.Builder
	...
	// Every project fragment, unioned — the shared squid serves them all.
	for _, pf := range ProjectFragments() {
		name := strings.TrimSuffix(strings.TrimPrefix(filepath.Base(pf), "project-"), ".acl")
		if err := appendFile("project "+name, pf); err != nil {
			return "", err
		}
	}
	if len(extra) > 0 {
